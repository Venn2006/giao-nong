...
import { ArrowLeft, MapPin, Plus, Minus, X, Star, ChevronRight, Store, Loader2, Clock, MessageSquare, Info, ShoppingCart, Coffee } from "lucide-react";
...
export default function ThucUongMenu() {
  ...
  const danhSachTopping = [
    { ten: "Trân châu đen", gia: 5000 },
    { ten: "Thạch phô mai", gia: 7000 },
    { ten: "Pudding trứng", gia: 5000 },
    { ten: "Kem macchiato", gia: 10000 }
  ];
  ...
  const handleCheckoutCustom = () => {
    if (!customQuan) { alert("Cô/Chú vui lòng điền giúp con tên quán nha!"); return; }
    const newItem = {
      id: Math.random().toString(),
      tenMon: monDangChon.name,
  ...
  if (isLoading) {
    return <div className="min-h-screen bg-[#fcfaf1] flex flex-col items-center justify-center max-w-md mx-auto"><Loader2 className="animate-spin text-orange-500 mb-4" size={40} /></div>;
  }

  // ==========================================
  // TRANG DANH SÁCH QUÁN BÁN NƯỚC (Giống Đồ Ăn)
  // ==========================================
  if (monDangChon) {
    const danhSachQuan = getDanhSachQuanNuoc(monDangChon.name);
    return (
      <div className="min-h-screen bg-[#fcfaf1] pb-10 font-sans max-w-md mx-auto shadow-2xl relative animate-in slide-in-from-right-8 duration-300">
        <header className="bg-white p-4 flex items-center gap-3 shadow-sm sticky top-0 z-10 rounded-b-2xl">
          <button onClick={() => { setMonDangChon(null); setQuanDangChon(null); setShowCustom(false); }} className="text-gray-600 active:bg-gray-200 p-2 rounded-full bg-gray-50"><ArrowLeft size={20}/></button>
          <div><h1 className="text-xl font-black text-gray-800 leading-tight">{monDangChon.name}</h1><p className="text-[10px] text-gray-500 font-bold">Chọn quán để mua</p></div>
        </header>
        ...
        <div className="p-4 space-y-5">
          {danhSachQuan.map((quan) => (
            <div key={quan.id} onClick={() => { setQuanDangChon(quan); setSoLuong(1); setSize("M"); setDuong("100%"); setDa("100%"); setToppings([]); setGhiChuMon(""); }} className="bg-white rounded-[2rem] overflow-hidden shadow-sm border border-gray-100 active:scale-[0.98] transition-transform cursor-pointer">
              ...
              <div className="p-5 flex justify-between items-center">
                <div><h3 className="text-xl font-black text-gray-800 mb-1">{quan.ten}</h3><p className="text-sm text-gray-500 font-medium flex items-center gap-1"><MapPin size={14} className="text-blue-500"/> {quan.diaChi} ({quan.khoangCach})</p></div>
          ...
          <div className="bg-gradient-to-r from-blue-600 to-cyan-500 rounded-[2rem] p-5 text-white shadow-lg relative overflow-hidden mt-6">
             ...
             <div className="relative z-10">
                ...
                <p className="text-xs font-medium text-blue-100 mb-4">Nhập tên tiệm trà sữa/cà phê bạn muốn, Shipper chạy đi mua ngay!</p>
                <button onClick={() => { setShowCustom(!showCustom); setSoLuong(1); setGhiChuMon(""); }} className="bg-white text-blue-600 font-black text-sm px-4 py-3 rounded-xl flex items-center gap-2 active:scale-95 transition-all shadow-sm w-full justify-center">
                  {showCustom ? "ĐÓNG FORM MUA HỘ" : <><Plus size={18}/> NHẬP TÊN QUÁN BẠN MUỐN</>}
                </button>
             </div>
          </div>
          ...
          {showCustom && (
            <div className="bg-white p-5 rounded-[2rem] border border-blue-100 shadow-lg space-y-4 animate-in fade-in slide-in-from-top-4">
               ...
               <input type="text" value={customQuan} onChange={(e) => setCustomQuan(e.target.value)} placeholder="Tên quán (VD: Mixue, Highlands)..." className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-blue-500 text-sm font-medium" />
               <input type="text" value={customAddress} onChange={(e) => setCustomAddress(e.target.value)} placeholder="Địa chỉ quán (Nếu biết)..." className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-blue-500 text-sm font-medium" />
               <div>
                  <textarea value={ghiChuMon} onChange={(e) => setGhiChuMon(e.target.value)} placeholder="Ví dụ: Lấy size L, ít đá, thêm thạch dừa..." className="w-full bg-gray-50 p-3 rounded-xl border border-gray-200 outline-blue-500 text-sm h-20 resize-none font-medium mt-2"></textarea>
               </div>
               <div className="flex justify-between items-center py-2 border-t border-gray-100 pt-4">
                  <span className="text-gray-800 font-bold text-sm">Số lượng phần</span>
                  <div className="flex items-center gap-3 bg-gray-50 p-1 rounded-xl border border-gray-200">
                    ...
                    <span className="font-black text-lg w-6 text-center text-gray-800">{soLuong}</span>
                    <button onClick={() => setSoLuong(soLuong + 1)} className="p-2 bg-white rounded-lg shadow-sm text-blue-600"><Plus size={16} /></button>
                  </div>
               </div>
               <button onClick={handleCheckoutCustom} className="w-full bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-black py-4 rounded-xl flex items-center justify-center gap-2 active:scale-95 shadow-md">
        ...
        {quanDangChon && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-end justify-center animate-in fade-in max-w-md mx-auto">
            <div className="bg-white w-full rounded-t-[2.5rem] p-6 slide-in-from-bottom-full duration-300 max-h-[90vh] overflow-y-auto">
              ...
              {/* CHỌN SIZE */}
              <div className="mb-5">
                 <h3 className="font-black text-gray-800 mb-2 text-sm uppercase">Chọn Size</h3>
                 <div className="flex gap-3">
                   ...
                   <button onClick={() => setSize("L")} className={`flex-1 py-3 rounded-xl font-black text-sm border-2 transition-all ${size === "L" ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-gray-50 border-gray-200 text-gray-500'}`}>Size L (+10.000đ)</button>
                 </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-5">
                 <div>
                   <h3 className="font-black text-gray-800 mb-2 text-sm uppercase">Lượng Đường</h3>
                   <div className="flex flex-col gap-2">
                     {['100%', '50%', 'Không đường'].map(lv => (
                       <button key={lv} onClick={() => setDuong(lv)} className={`py-2 rounded-lg font-bold text-xs border-2 ${duong === lv ? 'bg-orange-50 border-orange-400 text-orange-700' : 'bg-gray-50 border-gray-100 text-gray-500'}`}>{lv}</button>
                     ))}
                   </div>
                 </div>
                 <div>
                   <h3 className="font-black text-gray-800 mb-2 text-sm uppercase">Lượng Đá</h3>
                   <div className="flex flex-col gap-2">
                     {['100%', '50%', 'Không đá'].map(lv => (
                       <button key={lv} onClick={() => setDa(lv)} className={`py-2 rounded-lg font-bold text-xs border-2 ${da === lv ? 'bg-cyan-50 border-cyan-400 text-cyan-700' : 'bg-gray-50 border-gray-100 text-gray-500'}`}>{lv}</button>
                     ))}
                   </div>
                 </div>
              </div>
              ...
              {/* TOPPING */}
              <div className="mb-5">
                 <h3 className="font-black text-gray-800 mb-2 text-sm uppercase">Thêm Topping</h3>
                 <div className="space-y-2">
                   {danhSachTopping.map((top) => (
                     <label key={top.ten} className={`flex justify-between items-center p-3 rounded-xl border-2 cursor-pointer transition-colors ${toppings.includes(top.ten) ? 'bg-blue-50 border-blue-400' : 'bg-gray-50 border-gray-100'}`}>
                        <div className="flex items-center gap-3">
                           <input type="checkbox" checked={toppings.includes(top.ten)} onChange={() => toggleTopping(top.ten)} className="w-5 h-5 accent-blue-600 rounded" />
                           <span className="font-bold text-sm text-gray-800">{top.ten}</span>
                        </div>
              ...
              <div className="mb-6">
                <textarea value={ghiChuMon} onChange={(e) => setGhiChuMon(e.target.value)} placeholder="Ghi chú (Vd: Để đá riêng)..." className="w-full bg-gray-50 p-4 rounded-2xl border border-gray-200 outline-none text-sm h-20 resize-none font-medium"></textarea>
              </div>

              <div className="flex justify-between items-center mb-6">
                <span className="text-gray-800 font-bold text-lg">Số lượng ly</span>
              ...
              <button onClick={handleCheckout} className="w-full bg-gradient-to-r from-blue-600 to-cyan-500 text-white p-4 rounded-2xl shadow-lg flex justify-between items-center active:scale-[0.98] transition-transform">
                <div className="text-left"><p className="text-[10px] font-bold uppercase opacity-80 mb-0.5">Thêm vào giỏ</p><p className="text-2xl font-black">{(getGiaHienTai() * soLuong).toLocaleString('vi-VN')}đ</p></div>
  ...
  // ==========================================
  // TRANG MENU MẶC ĐỊNH (DANH MÚC THỨC UỐNG)
  // ==========================================
  ...
  return (
    <div className="min-h-screen bg-[#fcfaf1] pb-10 font-sans max-w-md mx-auto shadow-2xl relative">
      <header className="bg-white p-4 flex items-center gap-3 shadow-sm sticky top-0 z-10 rounded-b-2xl">
        ...
        <div><h1 className="text-xl font-black text-gray-800">{activeCategory || "Khám Phá Menu Nước"}</h1>{activeCategory && <p className="text-[10px] text-gray-500 font-bold">{dsMonTheoDanhMuc.length} món ngon giải khát</p>}</div>
      </header>
