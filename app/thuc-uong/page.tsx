...
export default function ThucUongMenu() {
  ...
  // THÊM VÀO GIỎ HÀNG NƯỚC (KHÔNG NHẢY TRANG)
  const handleAddToCartPopup = () => {
    ...
    // Đóng Popup
    setQuanDangChon(null);
    setMonDangChon(null);
    setSize("M"); setDuong("100%"); setDa("100%"); setToppings([]); setGhiChuMon(""); setSoLuong(1);
  };
  ...
  if (monDangChon) {
    ...
    return (
      <div className="min-h-screen bg-[#fcfaf1] pb-24 font-sans max-w-md mx-auto shadow-2xl relative animate-in slide-in-from-right-8 duration-300">
        ...
        <div className="p-4 space-y-5">
          {danhSachQuan.map((quan) => (
            <div key={quan.id} onClick={() => { setQuanDangChon(quan); setSoLuong(1); setSize("M"); setDuong("100%"); setDa("100%"); setToppings([]); setGhiChuMon(""); }} className="bg-white rounded-[2rem] overflow-hidden shadow-sm border border-gray-100 active:scale-[0.98] transition-transform cursor-pointer">
              ...
              <div className="p-5 flex justify-between items-center">
                <div><h3 className="text-xl font-black text-gray-800 mb-1">{quan.ten}</h3><p className="text-sm text-gray-500 font-medium flex items-center gap-1"><MapPin size={14} className="text-blue-500"/> {quan.diaChi}</p></div>
          ...
          {showCustom && (
            <div className="bg-white p-5 rounded-[2rem] border border-blue-100 shadow-lg space-y-4 animate-in fade-in slide-in-from-top-4">
               ...
               <input type="text" value={customQuan} onChange={(e) => setCustomQuan(e.target.value)} placeholder="Tên quán (VD: Highlands)..." className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-blue-500 text-sm font-medium" />
               <input type="text" value={customAddress} onChange={(e) => setCustomAddress(e.target.value)} placeholder="Địa chỉ quán..." className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-blue-500 text-sm font-medium" />
               <textarea value={ghiChuMon} onChange={(e) => setGhiChuMon(e.target.value)} placeholder="Ghi chú pha chế..." className="w-full bg-gray-50 p-3 rounded-xl border border-gray-200 outline-blue-500 text-sm h-20 resize-none font-medium mt-2"></textarea>
               <div className="flex justify-between items-center py-2 border-t border-gray-100 pt-4">
                  <span className="text-gray-800 font-bold text-sm">Số lượng</span>
        ...
        {quanDangChon && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-end justify-center animate-in fade-in max-w-md mx-auto">
            <div className="bg-white w-full rounded-t-[2.5rem] p-6 slide-in-from-bottom-full duration-300 max-h-[90vh] overflow-y-auto">
                 ...
                 <div className="space-y-2">
                   {danhSachTopping.map((top) => (
                     <label key={top.ten} className={`flex justify-between items-center p-3 rounded-xl border-2 cursor-pointer transition-colors ${toppings.includes(top.ten) ? 'bg-blue-50 border-blue-400' : 'bg-gray-50 border-gray-100'}`}>
                        <div className="flex items-center gap-3"><input type="checkbox" checked={toppings.includes(top.ten)} onChange={() => toggleTopping(top.ten)} className="w-5 h-5 accent-blue-600 rounded" /><span className="font-bold text-sm text-gray-800">{top.ten}</span></div>
              ...
              <div className="mb-6"><textarea value={ghiChuMon} onChange={(e) => setGhiChuMon(e.target.value)} placeholder="Ghi chú thêm..." className="w-full bg-gray-50 p-4 rounded-2xl border border-gray-200 outline-none text-sm h-20 resize-none font-medium"></textarea></div>
              ...
              <button onClick={handleAddToCartPopup} className="w-full bg-gradient-to-r from-blue-600 to-cyan-500 text-white p-4 rounded-2xl shadow-lg flex justify-between items-center active:scale-[0.98] transition-transform">
                ...
                <div className="flex items-center gap-1 font-black text-sm bg-black/20 px-4 py-2 rounded-xl backdrop-blur-sm"><ShoppingCart size={18}/> THÊM VÀO GIỎ</div>
              </button>
        ...
        {totalQty > 0 && !quanDangChon && (
          <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/90 backdrop-blur-md border-t border-gray-100 max-w-md mx-auto rounded-t-3xl z-40 animate-in slide-in-from-bottom-4">
            <button onClick={() => router.push('/thanh-toan')} className="w-full bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-black text-lg py-4 rounded-2xl active:scale-95 transition-all shadow-lg flex justify-between px-6 items-center">
              <span className="bg-white/20 px-3 py-1 rounded-lg text-sm">{totalQty} ly nước</span>
  ...
  return (
    <div className="min-h-screen bg-[#fcfaf1] pb-10 font-sans max-w-md mx-auto shadow-2xl relative">
      ...
      {totalQty > 0 && !activeCategory && !monDangChon && (
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/90 backdrop-blur-md border-t border-gray-100 max-w-md mx-auto rounded-t-3xl z-40 animate-in slide-in-from-bottom-4">
          <button onClick={() => router.push('/thanh-toan')} className="w-full bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-black text-lg py-4 rounded-2xl active:scale-95 transition-all shadow-lg flex justify-between px-6 items-center">
            <span className="bg-white/20 px-3 py-1 rounded-lg text-sm">{totalQty} món</span>
            <span>ĐẾN THANH TOÁN <ChevronRight size={18} className="inline"/></span>
          </button>
        </div>
      )}
