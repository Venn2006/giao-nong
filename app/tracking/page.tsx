...
export default function OrderTracking() {
  ...
  if (!order) {
    return (
      <div className="min-h-screen bg-[#fcfaf1] flex flex-col items-center justify-center p-6 text-center">
        ...
        <h2 className="text-xl font-black text-gray-900">Đang tìm đơn hàng của bạn...</h2>
      </div>
    );
  }
  ...
  const diemMuaHang = Math.floor((order.total_amount || 0) / 100000); // 100k = 1đ
  const diemDanhGia = 1;
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 font-sans max-w-md mx-auto shadow-2xl relative overflow-x-hidden">
      ...
      <header className={`p-4 flex items-center gap-3 sticky top-0 z-20 transition-colors ${step === 3 ? 'bg-white shadow-sm' : 'bg-transparent absolute w-full'}`}>
        <Link href="/">
          <button className="text-gray-900 active:bg-gray-200 p-2 rounded-full bg-white/80 backdrop-blur-md shadow-sm border border-gray-200">
            <ArrowLeft size={20} />
          </button>
        </Link>
        <h1 className="text-xl font-black text-gray-900 drop-shadow-sm">Đơn {order.order_code}</h1>
      </header>
      ...
      <div className={`pt-20 pb-12 px-6 rounded-b-[2.5rem] transition-colors duration-700 ${
        ...
        <div className="text-center mb-8">
          <h2 className="text-3xl font-black text-gray-900 mb-2 animate-in slide-in-from-bottom-2">
        ...
        <div className="relative flex justify-between items-center max-w-[280px] mx-auto z-10">
          <div className="absolute left-0 top-1/2 -translate-y-1/2 w-full h-1.5 bg-gray-300 rounded-full -z-10"></div>
          <div className="absolute left-0 top-1/2 -translate-y-1/2 h-1.5 bg-gray-900 rounded-full -z-10 transition-all duration-700" 
               style={{ width: step === 1 ? '0%' : step === 2 ? '50%' : '100%' }}></div>
          
          <div className={`w-10 h-10 rounded-full flex items-center justify-center border-4 border-white transition-colors duration-500 ${step >= 1 ? 'bg-gray-900 text-white' : 'bg-gray-200 text-gray-400'}`}>
            <ClipboardList size={16} />
          </div>
          <div className={`w-10 h-10 rounded-full flex items-center justify-center border-4 border-white transition-colors duration-500 ${step >= 2 ? 'bg-gray-900 text-white' : 'bg-gray-200 text-gray-400'}`}>
            <Motorbike size={16} className={step === 2 ? "animate-bounce" : ""} />
          </div>
          <div className={`w-10 h-10 rounded-full flex items-center justify-center border-4 border-white transition-colors duration-500 ${step >= 3 ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-400'}`}>
      ...
      <div className="p-4 space-y-4 -mt-6">
        ...
        {step >= 2 && order.shipper_name && (
          <div className="bg-white p-5 rounded-[2rem] shadow-lg border border-gray-100 flex items-center justify-between animate-in slide-in-from-bottom-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] text-gray-500 font-black uppercase mb-0.5">Tài xế của bạn</p>
                <p className="font-black text-gray-900 text-base leading-none">{displayShipperName}</p>
                {step === 2 && <p className="text-xs text-blue-600 font-black mt-1.5 flex items-center gap-1"><Motorbike size={12}/> Đang di chuyển tới bạn</p>}
              </div>
            </div>
            {step === 2 && (
        ...
        {/* ======================================================== */}
        {/* FIX LỖI: LÀM ĐẬM TOÀN BỘ CHỮ Ở PHẦN ĐÁNH GIÁ (FONT-BLACK) */}
        {/* ======================================================== */}
        {step === 3 && (
          <>
            {!isRated ? (
              <div className="bg-white p-6 rounded-[2rem] shadow-xl border-2 border-orange-400 animate-in slide-in-from-bottom-8">
                <h3 className="text-lg font-black text-gray-900 mb-2 text-center flex items-center justify-center gap-2">
                  <HeartHandshake className="text-orange-500" /> Đánh giá trải nghiệm
                </h3>
                <p className="text-xs text-gray-900 font-black text-center mb-6">Đánh giá để nhận <span className="text-orange-600">+1 điểm</span> thưởng Giao Nóng nhé!</p>
                ...
                <textarea 
                  ...
                  className="w-full p-4 bg-gray-50 border border-gray-300 rounded-2xl text-sm font-black text-gray-900 h-24 mb-4 outline-none focus:border-orange-500 resize-none placeholder-gray-500"
                />
              ...
              <div className="bg-green-50 p-6 rounded-[2rem] border-2 border-green-300 text-center shadow-lg animate-in zoom-in duration-500">
                ...
                <h3 className="text-xl font-black text-green-900 mb-2">Cảm ơn bạn đã đánh giá!</h3>
                {/* ĐÂY LÀ CHỖ MÀY CHỬI MỜ: ĐÃ SỬA THÀNH FONT-BLACK */}
                <p className="text-base text-green-900 mb-6 font-black">Đánh giá của bạn giúp Giao Nóng ngày càng tốt hơn.</p>

                <div className="bg-white p-4 rounded-2xl border-2 border-green-200 flex items-center justify-between shadow-sm">
                  <div className="flex items-center gap-2">
                    <Sparkles className="text-orange-500" size={24} />
                    <span className="font-black text-gray-900 text-base">Điểm nhận được:</span>
                  </div>
                  <span className="font-black text-3xl text-orange-600">+{tongDiemNhanDuoc}</span>
                </div>
                <p className="text-[11px] font-bold text-gray-600 mt-3 italic">*Đã bao gồm {diemMuaHang} điểm mua hàng và 1 điểm đánh giá</p>
        ...
        <div className="bg-white rounded-[2rem] shadow-sm border border-gray-200 overflow-hidden mt-4 relative">
          ...
          <div className="p-5 pt-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h4 className="font-black text-gray-900 text-base">Hóa đơn chi tiết</h4>
            </div>
            
            <div className="bg-gray-50 p-4 rounded-2xl mb-4 border border-gray-200">
              <p className="text-[10px] text-gray-600 font-black uppercase mb-1">Nội dung đơn</p>
              <p className="font-black text-gray-900 text-sm leading-relaxed whitespace-pre-line">
                {order.items_summary}
              </p>
            </div>
            ...
            <div className="border-t border-dashed border-gray-300 pt-4 space-y-2 mb-4">
              <div className="flex justify-between items-center">
                <span className="text-gray-700 text-sm font-black">Tổng tiền món</span>
                <span className="font-black text-gray-900 text-base">
                  {(order.total_amount - (order.shipping_fee || 0)).toLocaleString('vi-VN')}đ
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-700 text-sm font-black">Phí giao hàng</span>
                <span className="font-black text-gray-900 text-base">
            ...
            <div className="bg-gray-900 p-4 rounded-2xl flex justify-between items-center text-white shadow-inner">
              <div>
                <p className="text-[10px] text-gray-400 uppercase font-black tracking-wider mb-0.5">Tổng thanh toán</p>
                <p className="text-2xl font-black text-orange-400">{order.total_amount?.toLocaleString('vi-VN')}đ</p>
              </div>
              <div className="text-right">
                <p className="text-[10px] text-gray-400 uppercase font-black tracking-wider mb-0.5">Thanh toán</p>
                <p className="font-black text-sm bg-white/20 px-2 py-1 rounded-lg backdrop-blur-sm">
        ...
        <div className="bg-white p-5 rounded-[2rem] shadow-sm border border-gray-200 mb-8">
          <div className="flex items-center gap-2 mb-3">
            <Map size={18} className="text-gray-500" />
            <h4 className="font-black text-gray-800 text-sm uppercase tracking-wider">Thông tin giao hàng</h4>
          </div>
          <div className="pl-6 space-y-2 relative">
            <div className="absolute left-2.5 top-2 bottom-2 w-[2px] bg-gray-200"></div>
            
            <div className="relative">
              <div className="absolute -left-6 top-1 w-3 h-3 bg-gray-300 rounded-full border-2 border-white shadow-sm"></div>
              <p className="text-xs text-gray-500 font-black uppercase">Người nhận</p>
              <p className="font-black text-gray-900 text-sm">{order.customer_name} - {order.customer_phone}</p>
            </div>
            
            <div className="relative pt-2">
              <div className="absolute -left-6 top-3 w-3 h-3 bg-orange-500 rounded-full border-2 border-white shadow-sm"></div>
              <p className="text-xs text-gray-500 font-black uppercase">Giao đến</p>
              <p className="font-black text-gray-900 text-sm">{order.delivery_address}</p>
            </div>
          </div>
        </div>
