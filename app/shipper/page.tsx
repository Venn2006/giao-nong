...
import { 
  ...
  ClipboardList, Truck, XCircle, Banknote, QrCode, Navigation, Filter
} from "lucide-react";
...
export default function ShipperApp() {
  ...
  // STATE MỚI: LỌC TUYẾN ĐƯỜNG CHO NHIỀU TÀI XẾ
  ...
  // LOGIC LỌC TUYẾN (Dựa vào địa chỉ Khách hàng nhập)
  ...
  // TÍNH TOÁN BÁO CÁO (Dựa trên tuyến đang chọn)
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] font-sans pb-24 max-w-md mx-auto shadow-2xl relative">
      ...
      <header className="bg-blue-600 p-5 sticky top-0 z-30 shadow-md rounded-b-[2rem]">
        ...
        {/* BỘ LỌC TUYẾN ĐƯỜNG NẰM TRÊN HEADER */}
          ...
          {routes.map(route => (
            <button 
              ...
              className={`snap-start whitespace-nowrap px-4 py-2 rounded-xl text-sm font-black border-2 transition-colors ${
                ...
                : 'bg-blue-700/50 text-blue-100 border-blue-400/30'
              }`}
            >
              {route}
            </button>
      ...
      <div className="p-4 space-y-5 -mt-2">
        ...
        <div className="bg-white rounded-[1.5rem] p-4 shadow-sm border border-gray-100 flex justify-between items-center text-center">
           <div className="w-1/4">
              <ClipboardList size={20} className="mx-auto text-blue-500 mb-1"/>
              <p className="text-[10px] font-bold text-gray-500 uppercase">Tổng</p>
           ...
           <div className="w-1/4 border-l border-gray-100">
              ...
              <p className="text-[10px] font-bold text-gray-500 uppercase">Đang giao</p>
              <p className="text-xl font-black text-orange-600">{inProgressCount}</p>
           </div>
           <div className="w-1/4 border-l border-gray-100">
              <CheckCircle2 size={20} className="mx-auto text-green-500 mb-1"/>
              <p className="text-[10px] font-bold text-gray-500 uppercase">Đã giao</p>
              <p className="text-xl font-black text-green-600">{completedCount}</p>
           </div>
           <div className="w-1/4 border-l border-gray-100">
              <XCircle size={20} className="mx-auto text-red-500 mb-1"/>
              <p className="text-[10px] font-bold text-gray-500 uppercase">Hủy</p>
              <p className="text-xl font-black text-red-600">{cancelledCount}</p>
           </div>
        </div>
        ...
        <div className="bg-white rounded-[1.5rem] p-5 shadow-sm border border-gray-100">
           ...
           <div className="grid grid-cols-2 gap-4 mb-4">
              <div className="bg-orange-50 p-4 rounded-xl border border-orange-200">
                 <p className="text-[11px] font-black text-gray-600 uppercase flex items-center gap-1 mb-2"><Banknote size={14} className="text-orange-500"/> Tiền mặt nộp</p>
                 <p className="text-xl font-black text-orange-700">{cashToSubmit.toLocaleString('vi-VN')}đ</p>
              </div>
              <div className="bg-green-50 p-4 rounded-xl border border-green-200">
                 <p className="text-[11px] font-black text-gray-600 uppercase flex items-center gap-1 mb-2"><QrCode size={14} className="text-green-500"/> Đã chuyển khoản</p>
           ...
           <div className="bg-gray-900 p-4 rounded-xl flex justify-between items-center text-white shadow-md">
              <p className="text-xs font-black uppercase tracking-wider">Tổng Doanh Thu ({completedCount} đơn)</p>
          ...
          {activeOrders.map((order) => (
            <div key={order.id} className="bg-white rounded-[2rem] p-5 shadow-md border-2 border-gray-100">
              ...
              <div className="space-y-3 mb-5">
                 <div className="flex items-center gap-2 text-base font-black text-blue-900 ml-1">
                    <User size={20} className="text-blue-500" /> KH: {order.customer_name}
                 </div>
                 ...
                 <a href={`tel:${order.customer_phone}`} className="flex justify-between items-center bg-blue-50 text-blue-800 p-4 rounded-2xl text-base font-black border-2 border-blue-100 active:bg-blue-100 transition-colors">
                    ...
                    <span className="bg-blue-600 text-white text-xs px-3 py-1 rounded-lg">BẤM GỌI CHỐT ĐƠN</span>
                 </a>
              ...
              <div className="bg-gray-100 p-4 rounded-2xl border-2 border-gray-200 mb-5">
                 <p className="text-xs font-black text-gray-500 uppercase tracking-widest mb-2 border-b border-gray-300 pb-2">Danh sách món:</p>
              ...
              <div className="bg-gray-800 p-5 rounded-2xl flex justify-between items-center text-white mb-5 shadow-lg border-2 border-gray-700">
                 ...
                 <span className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider shadow-inner ${
                    order.payment_method === 'bank' ? 'bg-green-500 text-white' : 'bg-white text-gray-900'
                 }`}>
              ...
              <div className="space-y-3 mt-2">
                 {order.gps_location && (
                    <a href={`http://googleusercontent.com/maps.google.com/maps?daddr=${order.gps_location}&travelmode=motorcycle`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 bg-gray-100 text-blue-700 font-black w-full py-4 rounded-xl active:scale-95 text-sm uppercase border-2 border-gray-200 transition-colors hover:bg-gray-200">
                      <Navigation size={20} className="fill-blue-700" /> Bản đồ chỉ đường (Google Maps)
                    </a>
                 )}

                 {/* NÚT BẤM ĐÃ ĐỔI TÊN ĐỂ PHÙ HỢP VỚI MỌI TUYẾN HUYỆN */}
                 ...
                 {order.status === 'tx1_picking' && <button onClick={() => updateStatus(order.id, 'at_midpoint')} className="w-full bg-blue-600 text-white font-black py-5 rounded-2xl text-base active:scale-95 shadow-lg flex justify-center gap-2 border-[4px] border-blue-400"><MapPin size={20}/> [TX CÀ MAU] ĐÃ BỎ XUỐNG TRẠM</button>}
                 
                 {order.status === 'at_midpoint' && <button onClick={() => updateStatus(order.id, 'tx2_delivering')} className="w-full bg-orange-600 text-white font-black py-5 rounded-2xl text-base active:scale-95 shadow-lg flex justify-center gap-2"><Truck size={20}/> [TX TUYẾN HUYỆN] LẤY ĐI GIAO KHÁCH</button>}
                 {order.status === 'tx2_delivering' && <button onClick={() => updateStatus(order.id, 'completed')} className="w-full bg-green-600 text-white font-black py-5 rounded-2xl text-base active:scale-95 shadow-lg flex justify-center gap-2 border-[4px] border-green-400"><CheckCircle2 size={20}/> [TX TUYẾN HUYỆN] GIAO THÀNH CÔNG</button>}
              </div>
