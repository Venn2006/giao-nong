...
import { MapPin, Phone, RefreshCw, User, CheckCircle2, ClipboardList, Truck, Banknote, QrCode, Navigation, Package, Car, AlertTriangle, ShoppingBag, Calendar, Save } from "lucide-react";
...
export default function ShipperApp() {
  ...
  // TÀI KHOẢN TÀI XẾ (ĐỂ ĐẨY CHO KHÁCH XEM)
  const [shipperInfo, setShipperInfo] = useState({ name: "", phone: "" });
  ...
  useEffect(() => {
    // Kiểm tra thông tin tài xế trong máy
    const savedInfo = localStorage.getItem("giao_nong_shipper_info");
    if (savedInfo) {
       setShipperInfo(JSON.parse(savedInfo));
    } else {
       setShowProfileModal(true); // Ép nhập tên nếu chưa có
    }
  ...
  const handleSaveProfile = () => {
    if (!shipperInfo.name || !shipperInfo.phone) return alert("Nhập đủ Tên và SĐT nha người anh em!");
    localStorage.setItem("giao_nong_shipper_info", JSON.stringify(shipperInfo));
    setShowProfileModal(false);
  };

  // CẬP NHẬT TRẠNG THÁI ĐỒNG THỜI ĐẨY TÊN TÀI XẾ LÊN SERVER
  const updateStatus = async (id: string, newStatus: string) => {
    await supabase
      .from('orders')
      .update({ 
         status: newStatus,
         shipper_name: shipperInfo.name || "Tài xế Giao Nóng",
         shipper_phone: shipperInfo.phone || "0900000000"
      })
      .eq('id', id);
  };
    ...
    <div className="min-h-screen bg-[#f1f5f9] font-sans pb-24 max-w-md mx-auto shadow-2xl relative">
      ...
      {showProfileModal && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
           <div className="bg-white p-6 rounded-[2rem] w-full max-w-sm shadow-2xl animate-in zoom-in-95">
              ...
              <h2 className="text-xl font-black text-center text-gray-900 mb-2">Thông Tin Tài Xế</h2>
              <p className="text-xs font-bold text-center text-gray-500 mb-6">
                Nhập tên để Khách hàng biết ai đang giao đồ ăn nhé anh em!
              </p>
      ...
      <header className="bg-blue-600 p-5 sticky top-0 z-30 shadow-md rounded-b-[2rem]">
        <div className="flex justify-between items-center text-white mb-4">
          <div onClick={() => setShowProfileModal(true)} className="cursor-pointer active:opacity-70 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-blue-100 mt-0.5">{shipperInfo.name || 'Cài đặt hồ sơ'} ✏️</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-[1.5rem] p-5 shadow-sm border border-gray-100">
           ...
           <div className="grid grid-cols-2 gap-4 mb-4">
              <div className="bg-orange-50 p-4 rounded-xl border border-orange-200">
                <p className="text-[11px] font-black text-gray-600 uppercase flex items-center gap-1 mb-2"><Banknote size={14} className="text-orange-500"/> Tiền mặt giữ</p>
                <p className="text-xl font-black text-orange-700">{cashToSubmit.toLocaleString('vi-VN')}đ</p>
              </div>
              <div className="bg-green-50 p-4 rounded-xl border border-green-200">
                <p className="text-[11px] font-black text-gray-600 uppercase flex items-center gap-1 mb-2"><QrCode size={14} className="text-green-500"/> Khách CK Boss</p>
        ...
        <div className="space-y-5">
          {filteredByDateOrders.length === 0 ? (
            ...
            filteredByDateOrders.map((order) => {
              ...
              return (
                <div key={order.id} className={`bg-white rounded-[2rem] p-5 shadow-md border-2 ${isPackage ? 'border-blue-200' : isRide ? 'border-green-200' : isErrand ? 'border-purple-200' : 'border-gray-200'}`}>
                  <div className="flex justify-between items-start mb-5 border-b border-gray-100 pb-4">
                     <div>
                       ...
                       <p className={`text-[10px] font-black mt-1 uppercase flex items-center gap-1 ${isPackage ? 'text-blue-600' : isRide ? 'text-green-600' : isErrand ? 'text-purple-600' : 'text-orange-600'}`}>
                         {isPackage ? <Package size={12}/> : isRide ? <Car size={12}/> : isErrand ? <ShoppingBag size={12}/> : <MapPin size={12}/>}
                         {isPackage ? 'KIỆN HÀNG GOM TUYẾN' : isRide ? 'GỌI XE ÔM' : isErrand ? 'ĐƠN MUA HỘ ĐA NĂNG' : 'ĐỒ ĂN/NƯỚC NÓNG'}
                       </p>
                     </div>
                     <div className="flex flex-col items-end gap-1">
                       <span className={`px-3 py-1.5 rounded-lg text-[10px] font-black border-2 uppercase text-center min-w-[130px] max-w-[130px] leading-tight ${order.status === 'pending' ? 'bg-red-50 text-red-600 border-red-200 animate-pulse' : order.status === 'tx1_picking' ? 'bg-blue-50 text-blue-700 border-blue-200' : order.status === 'at_midpoint' ? 'bg-purple-50 text-purple-700 border-purple-200' : order.status === 'cancelled' ? 'bg-gray-100 text-gray-500 border-gray-200' : order.status === 'completed' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-orange-50 text-orange-700 border-orange-200'}`}>
                         ...
                         {order.status === 'at_midpoint' && '📍 ĐANG Ở BỜ ĐẬP'}
                         {order.status === 'tx2_delivering' && (isPackage ? '🚚 ĐANG CHẠY TUYẾN' : isRide ? '🚖 ĐANG CHỞ KHÁCH' : '🛵 ĐANG GIAO KHÁCH')}
                         {order.status === 'completed' && '✅ ĐÃ GIAO XONG'}
                         {order.status === 'cancelled' && '❌ ĐÃ HỦY ĐƠN'}
                       </span>
                       <span className="text-[9px] font-bold text-gray-400 bg-gray-50 px-2 py-0.5 rounded">{new Date(order.created_at).toLocaleDateString('vi-VN')}</span>
                     </div>
                  </div>
                  
                  <div className="space-y-3 mb-5">
                     <div className="flex items-center gap-2 text-base font-black text-gray-900 ml-1"><User size={20} className="text-gray-400" /> KH: {order.customer_name}</div>
                     ...
                     <div className="flex items-start gap-2 bg-gray-50 text-gray-900 p-4 rounded-2xl text-sm font-bold border-2 border-gray-200">
                        <MapPin size={20} className="text-red-500 mt-0.5 flex-shrink-0" /> <span className="leading-relaxed whitespace-pre-line">{order.delivery_address}</span>
                     </div>
                  </div>
                  ...
                  <div className="bg-yellow-50 p-4 rounded-2xl border-2 border-yellow-200 mb-5">
                     ...
                     {order.shipping_note && <p className="text-sm font-black text-red-600 mt-3 bg-white p-3 rounded-xl border border-red-200 shadow-sm">Lưu ý: {order.shipping_note}</p>}
                  </div>
                  ...
                  <div className="bg-gray-900 p-5 rounded-2xl flex justify-between items-center text-white mb-5 shadow-lg border-2 border-gray-700">
                     <div><p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Cần thu tiền</p><p className="text-4xl font-black text-yellow-400">{order.total_amount?.toLocaleString('vi-VN')}đ</p></div>
                     <span className={`px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider shadow-inner text-right ${order.payment_method === 'bank' ? 'bg-green-600 text-white' : 'bg-white text-gray-900'}`}>
                       {order.payment_method === 'bank' ? '🏦 KHÁCH ĐÃ CK' : order.payment_method === 'cash_sender' ? '💵 THU NGƯỜI GỬI' : order.payment_method === 'cash_receiver' ? '💵 THU NGƯỜI NHẬN' : '💵 THU TIỀN MẶT'}
                     </span>
                  </div>
                  ...
                  {['pending', 'tx1_picking', 'at_midpoint', 'tx2_delivering'].includes(order.status) && (
                    <div className="space-y-3 mt-2">
                      {order.gps_location && (
                          <a href={`https://maps.google.com/maps?daddr=${order.gps_location}&travelmode=driving`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 bg-gray-100 text-blue-700 font-black w-full py-4 rounded-xl active:scale-95 text-sm uppercase border-2 border-gray-300 transition-colors">
                      ...
                      {isFood && (
                        <>
                          ...
                          {order.status === 'tx1_picking' && <button onClick={() => updateStatus(order.id, 'at_midpoint')} className="w-full bg-blue-600 text-white font-black py-5 rounded-2xl text-base active:scale-95 shadow-lg border-[4px] border-blue-400"><MapPin className="inline mr-2" size={20}/> [CHẶNG 1] ĐÃ TỚI BỜ ĐẬP</button>}
                          {order.status === 'at_midpoint' && <div className="space-y-2"><p className="text-xs text-red-600 font-bold text-center flex items-center justify-center gap-1"><AlertTriangle size={14}/> 2 Tài xế đổi hàng tại Bờ Đập!</p><button onClick={() => updateStatus(order.id, 'tx2_delivering')} className="w-full bg-orange-600 text-white font-black py-5 rounded-2xl text-base active:scale-95 shadow-lg"><Truck className="inline mr-2" size={20}/> [CHẶNG 2] NHẬN TỪ BỜ ĐẬP, ĐI GIAO</button></div>}
                          {order.status === 'tx2_delivering' && <button onClick={() => updateStatus(order.id, 'completed')} className="w-full bg-green-600 text-white font-black py-5 rounded-2xl text-base active:scale-95 shadow-lg border-[4px] border-green-400"><CheckCircle2 className="inline mr-2" size={20}/> [CHẶNG 2] ĐÃ GIAO KHÁCH XONG</button>}
                        </>
                      )}
                      ...
                      {isPackage && (
                        <>
                          ...
                          {order.status === 'tx1_picking' && <button onClick={() => updateStatus(order.id, 'at_midpoint')} className="w-full bg-blue-600 text-white font-black py-5 rounded-2xl text-base active:scale-95 shadow-lg border-[4px] border-blue-400"><MapPin className="inline mr-2" size={20}/> [CHẶNG 1] MANG ĐẾN BỜ ĐẬP</button>}
                          {order.status === 'at_midpoint' && <div className="space-y-2"><p className="text-xs text-red-600 font-bold text-center flex items-center justify-center gap-1"><AlertTriangle size={14}/> 2 Tài xế đổi hàng tại Bờ Đập!</p><button onClick={() => updateStatus(order.id, 'tx2_delivering')} className="w-full bg-orange-600 text-white font-black py-5 rounded-2xl text-base active:scale-95 shadow-lg"><Truck className="inline mr-2" size={20}/> [CHẶNG 2] NHẬN TỪ BỜ ĐẬP, ĐI GIAO</button></div>}
                          {order.status === 'tx2_delivering' && <button onClick={() => updateStatus(order.id, 'completed')} className="w-full bg-green-600 text-white font-black py-5 rounded-2xl text-base active:scale-95 shadow-lg border-[4px] border-green-400"><CheckCircle2 className="inline mr-2" size={20}/> [CHẶNG 2] ĐÃ GIAO KIỆN XONG</button>}
                        </>
                      )}
                      ...
                      {isRide && (
                        <>
                          ...
                          {order.status === 'tx2_delivering' && <button onClick={() => updateStatus(order.id, 'completed')} className="w-full bg-green-600 text-white font-black py-5 rounded-2xl text-base active:scale-95 shadow-lg border-[4px] border-green-400"><CheckCircle2 className="inline mr-2" size={20}/> [TÀI XẾ] HOÀN THÀNH CUỐC XE</button>}
                        </>
                      )}
                      ...
                      {isErrand && (
                        <>
                          ...
                          {order.status === 'tx1_picking' && <button onClick={() => updateStatus(order.id, 'at_midpoint')} className="w-full bg-purple-600 text-white font-black py-5 rounded-2xl text-base active:scale-95 shadow-lg border-[4px] border-purple-400"><MapPin className="inline mr-2" size={20}/> [CHẶNG 1] MANG ĐẾN BỜ ĐẬP</button>}
                          {order.status === 'at_midpoint' && <div className="space-y-2"><p className="text-xs text-purple-600 font-bold text-center flex items-center justify-center gap-1"><AlertTriangle size={14}/> 2 Tài xế đổi hàng tại Bờ Đập!</p><button onClick={() => updateStatus(order.id, 'tx2_delivering')} className="w-full bg-orange-600 text-white font-black py-5 rounded-2xl text-base active:scale-95 shadow-lg border-[4px] border-orange-400"><Truck className="inline mr-2" size={20}/> [CHẶNG 2] NHẬN TỪ BỜ ĐẬP, ĐI GIAO</button></div>}
                          {order.status === 'tx2_delivering' && <button onClick={() => updateStatus(order.id, 'completed')} className="w-full bg-green-600 text-white font-black py-5 rounded-2xl text-base active:scale-95 shadow-lg border-[4px] border-green-400"><CheckCircle2 className="inline mr-2" size={20}/> [CHẶNG 2] GIAO KHÁCH THÀNH CÔNG</button>}
                        </>
                      )}
                    </div>
                  )}
